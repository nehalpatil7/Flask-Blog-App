...
let InsuranceClaim = (_dec = NearBindgen({}), _dec2 = view(), _dec3 = call({}), _dec(_class = (_class2 = class InsuranceClaim {
  claims = new UnorderedMap('unique-id-map1');
  // This method is read-only and can be called for free
  get_greeting() {
    if (this.claims.get("insurer_id") != null) {
      return this.claims.get("insurer_id") || null;
    } else {
      return null;
    }
  }
  ...
  set_greeting({
    ...
    if (this.claims.get(insurer_id) === null) {
      this.claims.set(insurer_id, new Vector('unique-id-vector1'));
    } else {
...
function set_greeting() {
  const _state = InsuranceClaim._getState();
  if (!_state && InsuranceClaim._requireInit()) {
    throw new Error("Contract must be initialized");
  }
  const _contract = InsuranceClaim._create();
  if (_state) {
    InsuranceClaim._reconstruct(_contract, _state);
  }
  const _args = InsuranceClaim._getArgs();
  const _result = _contract.set_greeting(_args);
  InsuranceClaim._saveToStorage(_contract);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(InsuranceClaim._serialize(_result, true));
}
